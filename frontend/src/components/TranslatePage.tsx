...
export function TranslatePage() {
  ...
  const imageTranslation = useImageTranslation(targetLanguage, sourceLanguage);
  const documentTranslation = useDocumentTranslation(targetLanguage, sourceLanguage);
  ...
  return (
    <div
      className={`mx-auto px-4 py-6 flex flex-col gap-4 transition-all duration-300 ${
        activeTab === "text" ? "max-w-2xl" : "max-w-4xl"
      }`}
    >
      {/* Tab Bar */}
      <div className="glass rounded-2xl p-1.5 flex gap-1 shadow-glass">
        {(
          [
            ...
            { id: "document", label: "Document", disabled: false },
          ] as const
        ).map((tab) => (
          <button
            key={tab.id}
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        {/* Auto-detecting status (image mode only) */}
        {activeTab === "image" &&
          imageTranslation.isProcessing &&
          sourceLanguage === undefined && (
            <span className="text-xs font-medium text-accent-cyan/80">
      ...
      {activeTab === "image" && (
        <ImageTranslatePanel
          imageFile={imageTranslation.imageFile}
          imagePreview={imageTranslation.imagePreview}
          translatedText={imageTranslation.translatedText}
          textType={imageTranslation.textType}
          isProcessing={imageTranslation.isProcessing}
          error={imageTranslation.error}
          handleFileSelect={imageTranslation.handleFileSelect}
          removeImage={imageTranslation.removeImage}
        />
      )}

      {activeTab === "document" && (
        <DocumentTranslatePanel {...documentTranslation} />
      )}
    </div>
  );
}
